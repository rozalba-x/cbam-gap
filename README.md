...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header {
            background: white; border-radius: 15px; padding: 30px; text-align: center; margin-bottom: 30px;
            box-shadow: 0 10px 30px rgba(0,0,0,0.1);
        }
        .header h1 { color: #2c3e50; font-size: 2.5em; margin-bottom: 10px; }
        .deadline-warning {
            background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; padding: 15px; border-radius: 10px;
            margin: 20px 0; text-align: center; font-weight: bold; font-size: 1.1em;
        }
        .financial-projection {
            background: linear-gradient(135deg, #8e44ad, #9b59b6); color: white; padding: 25px; border-radius: 15px; margin: 20px 0;
        }
        ...
        .albania-advantage {
            background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; padding: 25px; border-radius: 15px; margin: 20px 0;
        }
        ...
        .gap-analysis {
            background: white; border-radius: 15px; padding: 30px; margin: 20px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.1); display: none;
        }
        .gap-analysis.active { display: block; }
        .question-block { background: #f8f9fa; border-left: 5px solid #3498db; padding: 20px; margin: 15px 0; border-radius: 5px; }
        .question-title { font-weight: bold; color: #2c3e50; margin-bottom: 10px; font-size: 1.1em; }
        .status-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-top: 15px; }
        .status-option {
            padding: 10px; border-radius: 8px; text-align: center; cursor: pointer; border: 2px solid transparent; transition: all 0.3s ease;
        }
        ...
        .company-info { background: #e8f4fd; padding: 20px; border-radius: 10px; margin-bottom: 20px; }
        ...
        .info-input { padding: 8px 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; }
        .results-section {
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 25px; border-radius: 15px; margin-top: 30px;
        }
        ...
        .generate-btn {
            background: #27ae60; color: white; border: none; padding: 15px 30px; border-radius: 10px; font-size: 1.1em;
            cursor: pointer; margin: 20px auto; display: block; transition: all 0.3s ease;
        }
        .generate-btn:hover { background: #229954; transform: scale(1.05); }
        .kosovo-note { background: #d1ecf1; border: 1px solid #bee5eb; padding: 15px; border-radius: 8px; margin: 15px 0; }
        .cost-comparison { background: linear-gradient(135deg, #f39c12, #e67e22); color: white; padding: 20px; border-radius: 15px; margin: 20px 0; }
...
<div class="container">
    ...
    <div class="sector-selector">
        <div class="sector-btn" data-sector="steel" onclick="showSector('steel', this)">
            <div class="sector-icon">🏭</div>
            <h4>Çelik & Hekur</h4>
            <p>Tuba, profile, fletë</p>
        </div>
        <div class="sector-btn" data-sector="aluminium" onclick="showSector('aluminium', this)">
            <div class="sector-icon">⚡</div>
            <h4>Alumin</h4>
            <p>Pllaka, tela, profile</p>
        </div>
        <div class="sector-btn" data-sector="cement" onclick="showSector('cement', this)">
            <div class="sector-icon">🏗️</div>
            <h4>Çimento</h4>
            <p>Klinker, çimento i gatshëm</p>
        </div>
        <div class="sector-btn" data-sector="fertilizer" onclick="showSector('fertilizer', this)">
            <div class="sector-icon">🌱</div>
            <h4>Pleh</h4>
            <p>Bazuar në azot</p>
        </div>
    ...
    <div id="steel-analysis" class="gap-analysis">
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> Cili proces prodhimi? Furra e lartë, harku elektrik, DRI/EAF, apo rrugë tjetër? + % e skrapit</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Tracking i djegësve & % skrapit</small></div>
        ...
        <div class="question-block">
            ...
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Fatura & kWh/ton</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Vetëm konsumi total</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk kemi ndarje sipas produktit</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">3. Lëndë të Para Hyrëse & Emisionet e Integruara</div>
            <p><strong>CBAM kërkon:</strong> Xeheror/qymyr/skrap – origjina dhe CO₂ i integruar; % e skrapit në ngarkesë</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Çertifikata & origjina</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Origjina e njohur, pa CO₂</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk kemi të dhëna të furnizuesve</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">4. Komunikimi me BE & Template</div>
            <p><strong>CBAM kërkon:</strong> Plotësimi i template-it të Komisionit & dërgimi te importuesit</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Template i njohur & përdorur</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Dimë për të</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk e kemi përdorur</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">5. Çmimi i Karbonit në Shqipëri</div>
            <p><strong>Avantazhi CBAM:</strong> A paguani tashmë çmime CO₂? (Zbriten nga CBAM me provë)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ Paguajmë çmim CO₂<br><small>E dokumentuar</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Taksa/ tarifa mjedisore</small></div>
    ...
    <div id="aluminium-analysis" class="gap-analysis">
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> Elektrolizë (primar) apo riciklim? % e skrapit në përzierje</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Rruga + % skrapit</small></div>
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> kWh/kg alumin + burimi (hidro = avantazh shqiptar)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>kWh/kg & çertifikim burimi</small></div>
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> CF₄ & C₂F₆ nga elektroliza (vetëm primar)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>PFC të matura</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Vlerësime/ faktorë default</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi të dhëna PFC</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">4. Origjina e Aluminës (Alumina)</div>
            <p><strong>CBAM kërkon:</strong> CO₂ i integruar në aluminë (nëse importohet)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Të dhënat CO₂ nga furnizuesit</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Origjina e njohur</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk kemi CO₂ të integruar</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">5. Komunikimi me BE & Template</div>
            <p><strong>CBAM kërkon:</strong> Plotësim & dërgim i template-it te importuesit e BE</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Template i njohur & përdorur</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Nevojitet trajnim</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi kapacitet</small></div>
            </div>
        </div>
    </div>
    ...
    <div id="cement-analysis" class="gap-analysis">
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> CO₂ nga dekarbonizimi i CaCO₃ (~60% e totalit)</p>
            <div class="status-grid">
                ...
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi të dhëna procesi</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">2. Emisionet nga Djegja</div>
            <p><strong>CBAM kërkon:</strong> CO₂ nga karburantet (qymyr, gaz, alternativë) për ton klinker</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Balanca e karburanteve</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Të dhëna vjetore</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk kemi ndarje sipas furrave</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">3. Raporti Klinker-Çimento</div>
            <p><strong>CBAM kërkon:</strong> Sa klinker për ton çimento të gatshëm? (Shtesat/SCM ulin intensitetin)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Recepturat e sakta</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Vlera mesatare</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Nuk kemi ndarje</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">4. Materiale Plotësuese (SCM) & Karbonati i Kalkinuar</div>
            <p><strong>CBAM kërkon:</strong> Sasi & lloj (slag, fly ash, metakaolin, klinker zëvendësues)</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Dokumentim i plotë i SCM-ve</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Vetëm % indikative</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Mungon dokumentimi</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">5. MRV & Template i BE (Klinker/Çimento)</div>
            <p><strong>CBAM kërkon:</strong> Metodologji e matjes/ verifikimit & aftësi për ta raportuar sipas template-it</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>MRV & template gati</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>MRV bazik, nevojitet trajnim</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi MRV/template</small></div>
            </div>
        </div>
    </div>
    ...
    <div id="fertilizer-analysis" class="gap-analysis">
        ...
        <div class="question-block">
            ...
            <p><strong>CBAM kërkon:</strong> N₂O nga acidi nitrik/ nitratet + CO₂ nga gazi natyror/ amoniaku</p>
            <div class="status-grid">
                ...
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi të dhëna emisionesh</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">2. Origjina e Amoniakut</div>
            <p><strong>CBAM kërkon:</strong> CO₂ i integruar në amoniakun e importuar (nëse nuk prodhohet vetë)</p>
            <div class="status-grid">
        ...
        <div class="question-block">
            <div class="question-title">3. Efikasiteti i Energjisë & Karburantit</div>
            <p><strong>CBAM kërkon:</strong> kWh/ton amoniak/ure, lloji i karburantit, rikuperim nxehtësie</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>kWh/ton & karburanti</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Të dhëna vjetore</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Pa ndarje sipas linjave</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">4. Lloji i Produktit & Përmbajtja e Karbonit</div>
            <p><strong>CBAM kërkon:</strong> Ure/AN/CAN/NP/NPK – përmbajtje azoti, rrjedhimisht profil CO₂/ton produkt</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Specifikimet e produkteve</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Vetëm total prodhimi</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>Mungon ndarja sipas produktit</small></div>
            </div>
        </div>

        <div class="question-block">
            <div class="question-title">5. Komunikimi me BE & Template</div>
            <p><strong>CBAM kërkon:</strong> Plotësim & dërgim i template-it te importuesit</p>
            <div class="status-grid">
                <div class="status-option status-have" onclick="selectStatus(this)">✅ E kemi<br><small>Template i njohur & përdorur</small></div>
                <div class="status-option status-partial" onclick="selectStatus(this)">⚠️ Pjesërisht<br><small>Nevojitet trajnim</small></div>
                <div class="status-option status-need" onclick="selectStatus(this)">❌ Na mungon<br><small>S’kemi kapacitet</small></div>
...
<script>
    ...
    function showSector(sector, el) {
        // Hide all analyses
        document.querySelectorAll('.gap-analysis').forEach(a => a.classList.remove('active'));
        // Deactivate all buttons
        document.querySelectorAll('.sector-btn').forEach(b => b.classList.remove('active'));
        // Show selected analysis
        const panel = document.getElementById(sector + '-analysis');
        if (panel) panel.classList.add('active');
        // Activate clicked button (if provided)
        if (el) el.classList.add('active');
        else {
            // Try activate by data attribute (when opening from dropdown/load)
            const btn = document.querySelector(`.sector-btn[data-sector="${sector}"]`);
            if (btn) btn.classList.add('active');
        }
        currentSector = sector;
        responses[sector] = responses[sector] || {};
        restoreSelectionsForSector(sector);
        saveProgress();
    }
    ...
    function selectStatus(element) {
        ...
        const questionTitle = questionBlock.querySelector('.question-title').textContent.trim();
        const status = element.classList.contains('status-have') ? 'have'
                    : element.classList.contains('status-partial') ? 'partial'
                    : 'need';
    ...
    function generateResults() {
        ...
        // Count only answered questions (so users see live score while progressing)
        const sectorResponses = responses[currentSector] || {};
        const answered = Object.keys(sectorResponses).length;
        if (answered === 0) { alert('Ju lutem përgjigjuni pyetjeve fillimisht!'); return; }
        ...
        const readinessScore = Math.round(((haveCount * 2 + partialCount) / (answered * 2)) * 100);
        ...
        if (readinessScore >= 80) {
            actionPlan += `<div class="action-item priority-low"><strong>🎉 Shkëlqyeshëm!</strong> Jeni mirë të përgatitur. Fokus në template & komunikim me importuesit e BE-së.</div>`;
        } else if (readinessScore >= 50) {
            actionPlan += `<div class="action-item priority-medium"><strong>⚠️ Fillim i mirë!</strong> Mbyllni hendekun e të dhënave deri në fund të 2025. Rekomandohet programi pilot.</div>`;
        } else {
            actionPlan += `<div class="action-item priority-high"><strong>🚨 Urgjent!</strong> Masa të menjëhershme të nevojshme. Programi Pilot CBAM i domosdoshëm për 2026.</div>`;
        }

        // Sector-specific nudges
        if (currentSector === 'steel') {
            actionPlan += `<div class="action-item priority-medium"><strong>🏭 Specifike për Çelik:</strong> Dokumentoni rrugën (BF/BOF, EAF, DRI/EAF), % e skrapit, origjinat e lëndëve, përdorni hidroenergjinë si avantazh.</div>`;
        } else if (currentSector === 'aluminium') {
            actionPlan += `<div class="action-item priority-medium"><strong>⚡ Specifike për Alumin:</strong> Çertifikoni hidroenergjinë, maturi PFC për primarin, gjurmoni % skrapit.</div>`;
        } else if (currentSector === 'cement') {
            actionPlan += `<div class="action-item priority-medium"><strong>🏗️ Specifike për Çimento:</strong> Ndarje proces vs djegje, rritje SCM-ve (slag/pozzolan), dokumentoni karburantët alternativë.</div>`;
        } else if (currentSector === 'fertilizer') {
            actionPlan += `<div class="action-item priority-high"><strong>🌱 Specifike për Pleh:</strong> N₂O është kritike; kërkoni CO₂ të integruar për amoniakun dhe optimizoni energjinë.</div>`;
        }

        // Albania angle
        actionPlan += `<div class="action-item priority-low"><strong>🇦🇱 Avantazhi i Shqipërisë:</strong> Emisione të ulëta nga energjia (0,1 kg CO₂/kWh) – monetizoni këtë te importuesit e BE-së.</div>`;
        actionPlan += `<div class="action-item priority-high"><strong>⚠️ Kujdes:</strong> Proceset termike (gaz, naftë, qymyr) llogariten veçmas – shpesh vendi me kostot më të larta.</div>`;
        ...
        // Costs calculator
        const exportVolume = parseFloat(document.getElementById('export-volume').value) || 1000;
        let sectorEmissionFactor = 1.5; // default steel
        ...
        const priceCO2 = 120; // €/t CO2 (2026 baseline)
        const preparedCost = Math.round(exportVolume * sectorEmissionFactor * priceCO2);
        const unpreparedCost = Math.round(preparedCost * 1.5); // +50% default
        const savings = unpreparedCost - preparedCost;

        actionPlan += `
            <div class="action-item priority-high">
                <strong>💰 Analiza e Kostove për ${exportVolume.toLocaleString()} ton/vit:</strong>
                <ul style="line-height: 1.8; margin: 10px 0;">
        ...
        // 10-year simple projection (conservative multiplier for rising price)
        const totalSavings10Years = Math.round(savings * 8);
        actionPlan += `
            <div class="action-item priority-high">
    ...
    function loadProgress() {
        ...
        document.getElementById('main-product').value = c.product || '';
        document.getElementById('export-volume').value = c.volume || '';
        document.getElementById('eu-buyers').value = c.buyers || '';
        if (data.sector) showSector(data.sector);
        restoreSelectionsForSector(data.sector);
    }

    function restoreSelectionsForSector(sector) {
        if (!sector || !responses[sector]) return;
        // Clear selections in visible panel
        const panel = document.getElementById(sector + '-analysis');
        if (!panel) return;
        panel.querySelectorAll('.question-block').forEach(qb => {
            qb.querySelectorAll('.status-option').forEach(opt => opt.classList.remove('selected'));
            const title = qb.querySelector('.question-title')?.textContent.trim();
            const state = responses[sector][title];
            if (!state) return;
            const selector = state === 'have' ? '.status-have' : state === 'partial' ? '.status-partial' : '.status-need';
            const target = qb.querySelector(selector);
            if (target) target.classList.add('selected');
        });
    }

    // Auto-map dropdown to sector
    document.addEventListener('DOMContentLoaded', () => {
        loadProgress();
        document.getElementById('main-product').addEventListener('change', function(){
            const sector = this.value;
            if (sector) showSector(sector);
            saveProgress();
        });
        // If no sector selected, default to first button for UX
        if (!currentSector) {
            const firstBtn = document.querySelector('.sector-btn');
